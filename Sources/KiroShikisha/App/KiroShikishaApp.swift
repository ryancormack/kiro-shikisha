                    ...
                    await withTaskGroup(of: Void.self) { group in
                        for restoredTask in taskManager.allTasks {
                            if restoredTask.status == .paused && restoredTask.sessionId != nil {
                                let taskName = restoredTask.name
                                let taskId = restoredTask.id
                                group.addTask {
                                    print("[TaskReconnect] Starting reconnect for: \(taskName)")
                                    do {
                                        try await withThrowingTaskGroup(of: Void.self) { inner in
                                            inner.addTask {
                                                try await taskManager.reopenTask(id: taskId)
                                            }
                                            inner.addTask {
                                        ...
                                        print("[TaskReconnect] Successfully reconnected: \(taskName)")
                                    } catch {
                                        print("[TaskReconnect] Failed for \(taskName): \(error)")
                                    }
                                }
                            }
                        }
